function moduleProject3() {
  ...
  function buildLearnerCard(learner, languages) {
    ...
    const favPLang = document.createElement('p')
    const favLanguage = languages.find(lang => lang.id === learner.favLanguage)
    favPLang.textContent = `Favorite Language: ${favLanguage.name}`;

    [nameP, idElement, dobP, favPLang].forEach(p => {
      card.appendChild(p)
    })
  ...
  function buildFooter(footerData) {
    ...
    // using 'innteraHTML' because of the 'a href' link
